...
To keep things simple, let's take the plugin from the [previous section](./react.mdx) and try to separate it out and build it with Webpack. If we identify the parts of that plugin, we end up with the meta comment, the react component, and the main plugin class. So that corresponds to three different files shown below.
...
```json title=".babelrc" showLineNumbers
{
  "presets": [
    [
      "@babel/env",
...
```jsx title="src/component.jsx" showLineNumbers
export default function MyComponent({disabled = false}) {
    const [isDisabled, setDisabled] = BdApi.React.useState(disabled);
